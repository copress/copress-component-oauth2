...
module.exports = function token(options, issue) {
    ...
    function request(req) {
        ...
        if (typeof clientID !== 'number') {
            throw new AuthorizationError('Parameter `client_id` type is invalid, expect number', 'invalid_request');
        }
    ...
    function response(txn, res, next) {
        if (!txn.redirectURI) {
            return next(new Error('Unable to issue redirect for OAuth 2.0 transaction'));
        }
        if (!txn.res.allow) {
            var err = {};
