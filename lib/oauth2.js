 ...
 * Module dependencies.
 */
var debug = require('debug')('sycle:oauth2')
    , url = require('url')
...
module.exports = function (app, options) {
    options = options || {};
    var sapp = app.sapp;
    var models = modelBuilder(sapp, options);
            ...
            function (client, code, redirectURI, done) {
                ...
                models.AuthorizationCodes.findByCode(code, function (err, authCode) {
                    ...
                    models.AuthorizationCodes.delete(code, function (err) {
                        ...
                        models.AccessTokens.save(token.token, clientId,
    ...
    function getTokenHandler(params, done) {
        return function (err, accessToken) {
            ...
            done(null, accessToken.token, helpers.buildTokenParams(accessToken, params));
        };
    }
        ...
        server.exchange(oauth2Provider.exchange.password(
            function (client, username, password, scope, done) {
                ...
                userLogin(username, password, function (err, user) {
                    ...
                    models.AccessTokens.save(token.token, client.id, user.id,
            ...
            function (client, subject, scope, done) {
                ...
                function generateAccessToken(user) {
                    ...
                    models.AccessTokens.save(
                        token.token, client.id, user && user.id, scope, refreshToken,
                        getTokenHandler(token, done));
                }
            ...
            function (client, refreshToken, scope, done) {
                ...
                models.AccessTokens.findByRefreshToken(refreshToken,
                    function (err, accessToken) {
                        ...
                        models.AccessTokens.save(token.token, client.id, accessToken.userId,
                            scope, refreshToken, getTokenHandler(token, done));
                    });
            }));
    }
        ...
        tokenGrant = server.grant(oauth2Provider.grant.token(
            ...
            function (client, user, scope, ares, done) {
                ...
                function generateAccessToken() {
                    ...
                    models.AccessTokens.save(token.token, client.id, user.id, scope, null,
                        getTokenHandler(token, done));
                }
        ...
        server.exchange('urn:ietf:params:oauth:grant-type:jwt-bearer',
            oauth2Provider.exchange.jwt(function (client, jwtToken, done) {
                ...
                function generateAccessToken(user) {
                    ...
                    models.AccessTokens.save(
                        token.token, client.id, user && user.id, payload.scope, null,
                        getTokenHandler(token, done));
                }
    ...
    handlers.token = [
        passport.authenticate(
            ['sycle-oauth2-client-password',
                'sycle-oauth2-client-basic',
                'sycle-oauth2-jwt-bearer'],
            {session: false}),
        server.token(),
        server.errorHandler()
    ];
    ...
    // HTTP basic
    passport.use('sycle-oauth2-client-basic', new BasicStrategy(clientLogin));
    // Body
    passport.use('sycle-oauth2-client-password',
        new ClientPasswordStrategy(clientLogin));

    /**
     * JWT bearer token
     */
    passport.use('sycle-oauth2-jwt-bearer', new ClientJWTBearerStrategy(
        {
    ...
    app.middleware('parse', oauth2Paths,
        app.expressx.urlencoded({extended: false}));
    app.middleware('parse', oauth2Paths, app.expressx.json({strict: false}));
    ...
    if (options.loginPath !== false) {
         ...
         */
        passport.use('sycle-oauth2-local', new LocalStrategy(userLogin));
        ...
        app.post(options.loginPath || '/login',
            passport.authenticate('sycle-oauth2-local',
                {
