 ...
 * Module dependencies.
 */
var debug = require('debug')('copress:oauth2')
    , url = require('url')
...
module.exports = function (app, options) {
    ...
    var generateToken = options.generateToken || function (options) {
            ...
            if (options.client && options.client.tokenType === 'jwt') {
                ...
                var payload = {
                    id: id,
                    clientId: options.client.id.toString(), // TODO Is this necessary to convert ?
                    userId: options.user && options.user.id.toString(), // TODO Is this necessary to convert ?
                    scope: options.scope,
                    createdAt: new Date()
                };
    ...
    handlers.token = [
        passport.authenticate(
            ['copress-oauth2-client-password',
                'copress-oauth2-client-basic',
                'copress-oauth2-jwt-bearer'],
            {session: false}),
        server.token(),
        server.errorHandler()
    ];
    ...
    // HTTP basic
    passport.use('copress-oauth2-client-basic', new BasicStrategy(clientLogin));
    // Body
    passport.use('copress-oauth2-client-password',
        new ClientPasswordStrategy(clientLogin));

    /**
     * JWT bearer token
     */
    passport.use('copress-oauth2-jwt-bearer', new ClientJWTBearerStrategy(
        {
    ...
    if (options.loginPath !== false) {
         ...
         */
        passport.use('copress-oauth2-local', new LocalStrategy(userLogin));
        ...
        app.post(options.loginPath || '/login',
            passport.authenticate('copress-oauth2-local',
                {
